...
export const volumeUnattributed = (days = 14) => api.get('/volume/unattributed', { params: { days } }).then(r => r.data);
const exclParam = (exclude) => (Array.isArray(exclude) && exclude.length ? exclude.join(',') : undefined);
export const volumeTrend       = (period = 'week', date = null, exclude = null, offset = 0) =>
  api.get('/volume/trend', { params: { period, date, offset, exclude: exclParam(exclude) } }).then(r => r.data);
export const volumeLeaderboard = ({ period = 'month', metric = 'parcels', sort = 'growth', limit = 6, date = null, exclude = null, offset = 0 } = {}) =>
  api.get('/volume/leaderboard', { params: { period, metric, sort, limit, date, offset, exclude: exclParam(exclude) } }).then(r => r.data);
